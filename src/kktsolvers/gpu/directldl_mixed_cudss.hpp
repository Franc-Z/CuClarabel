...
public:
    ...
    CUDSSDirectLDLSolverMixed(const std::vector<int>& KKT, std::vector<T>& x, std::vector<T>& b)
        : KKTgpu(KKT), x(x), b(b) {
...
std::map<std::string, CUDSSDirectLDLSolverMixed> GPUSolversDict = { {"cudssmixed", CUDSSDirectLDLSolverMixed()} };

std::string required_matrix_shape(const std::type_info& type) {
    if (type == typeid(CUDSSDirectLDLSolverMixed)) {
        return "full";
    }
    return "";
}
