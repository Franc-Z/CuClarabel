...
void unit_initialization_psd(
    ...
    T alpha = T(1);
    scaled_unit_shift_psd(z, alpha, rng_cones, psd_dim, n_shift, n_psd);
    scaled_unit_shift_psd(s, alpha, rng_cones, psd_dim, n_shift, n_psd);
}
...
template <typename T>
bool update_scaling_psd(
    thrust::device_vector<T>& L1,
    thrust::device_vector<T>& L2,
    thrust::device_vector<T>& z,
    thrust::device_vector<T>& s,
    thrust::device_vector<T>& workmat1,
    thrust::device_vector<T>& lambda_psd,
    thrust::device_vector<T>& lambda_isqrt,
    thrust::device_vector<T>& R,
    thrust::device_vector<T>& Rinv,
    thrust::device_vector<T>& Hspsd,
    thrust::device_vector<int>& rng_cones,
    ...
    thrust::copy(S.begin(), S.end(), lambda_psd.begin());
    thrust::transform(lambda_psd.begin(), lambda_psd.end(), lambda_isqrt.begin(), thrust::placeholders::_1 = 1 / sqrt(thrust::placeholders::_1));
    ...
    right_mul_batched(R, lambda_isqrt, R);
    ...
    left_mul_batched(lambda_isqrt, Rinv, Rinv);
    cublasDestroy(cublas_handle);
...
void combined_ds_shift_psd(
    ...
    scaled_unit_shift_psd(shift, -sigma_mu, rng_cones, psd_dim, n_shift, n_psd);
}

template <typename T>
__global__ void _kernel_op_lambda(
    T* X,
    T* Z,
    T* lambda_psd,
    int psd_dim,
    ...
    if (i <= n_psd) {
        ...
        T* lambdai = &lambda_psd[i * psd_dim];
        for (int k = 0; k < psd_dim; ++k) {
            for (int j = 0; j < psd_dim; ++j) {
                Xi[k * psd_dim + j] = 2 * Zi[k * psd_dim + j] / (lambdai[k] + lambdai[j]);
            }
        }
    }
}
...
void op_lambda(
    ...
    _kernel_op_lambda<<<blocks, threads>>>(
        thrust::raw_pointer_cast(X.data()),
        thrust::raw_pointer_cast(Z.data()),
        thrust::raw_pointer_cast(lambda_psd.data()),
        psd_dim,
        n_psd
    );
    cudaDeviceSynchronize();
...
template <typename T>
void delta_s_from_delta_z_offset_psd(
    CompositeConeGPU<T>& cones,
    ...
    thrust::device_vector<T>& R = cones.R;
    thrust::device_vector<T>& lambda_psd = cones.lambda_psd;
    thrust::device_vector<int>& rng_cones = cones.rng_cones;
    thrust::device_vector<T>& workmat1 = cones.workmat1;
    thrust::device_vector<T>& workmat2 = cones.workmat2;
    thrust::device_vector<T>& workmat3 = cones.workmat3;
    ...
    op_lambda(workmat1, workmat2, lambda_psd, psd_dim, n_psd);
...
T step_length_psd(
    ...
    thrust::device_vector<T>& work_delta = workmat1;

    mul_Wx_psd(d, dz, Rx, rng_cones, workmat1, workmat2, workmat3, n_shift, n_psd, true);
    T alpha_z = step_length_psd_component_gpu(work_delta, d, lambda_isqrt, n_psd, alpha_max);

    mul_WTx_psd(d, ds, Rinv, rng_cones, workmat1, workmat2, workmat3, n_shift, n_psd, true);
    T alpha_s = step_length_psd_component_gpu(work_delta, d, lambda_isqrt, n_psd, alpha_max);

    return std::min({alpha_max, alpha_z, alpha_s});
}
...
T compute_barrier_psd(
    ...
    T barrier_d = _logdet_barrier_psd(barrier, z, dz, alpha, workmat1, workvec, rng_cones, psd_dim, n_psd);
    T barrier_p = _logdet_barrier_psd(barrier, s, ds, alpha, workmat1, workvec, rng_cones, psd_dim, n_psd);
...
T step_length_psd_component_gpu(
    thrust::device_vector<T>& work_delta,
    thrust::device_vector<T>& d,
    thrust::device_vector<T>& lambda_isqrt,
    int n_psd,
    T alpha_max
) {
    svec_to_mat_no_shift_gpu(work_delta, d, n_psd);
    lrscale_gpu(work_delta, lambda_isqrt);

    thrust::device_vector<T> e = cusolverDnSyevjBatched('N', 'U', work_delta);

    T gamma = thrust::reduce(e.begin(), e.end(), std::numeric_limits<T>::max(), thrust::minimum<T>());
    if (gamma < 0) {
        return std::min(1 / -gamma, alpha_max);
    } else {
        return alpha_max;
    }
}
