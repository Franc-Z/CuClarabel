...
template <typename T>
__global__ void _kernel_unit_initialization_exp(
    T* z,
    T* s,
    int* rng_cones,
    int n_shift,
...
void unit_initialization_exp(
    ...
    int threads = std::min(n_exp, 1024);
    int blocks = (n_exp + threads - 1) / threads;

    _kernel_unit_initialization_exp<<<blocks, threads>>>(
...
template <typename T>
__global__ void _kernel_update_scaling_exp(
    T* s,
    T* z,
    T* grad,
    T* Hs,
...
void update_scaling_exp(
    ...
    _kernel_update_scaling_exp<<<blocks, threads>>>(
        thrust::raw_pointer_cast(s.data()),
        thrust::raw_pointer_cast(z.data()),
...
template <typename T>
__global__ void _kernel_get_Hs_exp(
    T* Hsblock,
    T* Hs,
    int* rng_blocks,
    int n_shift,
...
void get_Hs_exp(
    ...
    _kernel_get_Hs_exp<<<blocks, threads>>>(
...
template <typename T>
__global__ void _kernel_combined_ds_shift_exp(
    T* shift,
    T* step_z,
    T* step_s,
    T* z,
...
void combined_ds_shift_exp(
    ...
    _kernel_combined_ds_shift_exp<<<blocks, threads>>>(
...
template <typename T>
__global__ void _kernel_step_length_exp(
    T* dz,
    T* ds,
    T* z,
    T* s,
...
T step_length_exp(
    ...
    _kernel_step_length_exp<<<blocks, threads>>>(
    ...
    return thrust::reduce(α.begin(), α.begin() + n_exp, αmax, thrust::minimum<T>());
}

template <typename T>
T backtrack_search_exp(
    T* dz,
    T* z,
    T* ds,
    T* s,
...
template <typename T>
__global__ void _kernel_compute_barrier_exp(
    T* barrier,
    T* z,
    T* s,
    T* dz,
    ...
    if (i <= n_exp) {
        ...
        T barrier_d = barrier_dual_exp(cur_z);
        T barrier_p = barrier_primal_exp(cur_s);
        barrier[i] = barrier_d + barrier_p;
    }
}

template <typename T>
T compute_barrier_exp(
    ...
    _kernel_compute_barrier_exp<<<blocks, threads>>>(
    ...
    return thrust::reduce(barrier.begin(), barrier.begin() + n_exp, static_cast<T>(0), thrust::plus<T>());
}

template <typename T>
T barrier_dual_exp(T* z) {
    T l = log(-z[2] / z[0]);
    return -log(-z[2] * z[0]) - log(z[1] - z[0] - z[0] * l);
}

template <typename T>
T barrier_primal_exp(T* s) {
...
template <typename T>
bool is_primal_feasible_exp(T* s) {
    if (s[2] > 0 && s[1] > 0) {
...
template <typename T>
void gradient_primal_exp(T* s, T* grad) {
    T ω = _wright_omega_gpu(1 - s[0] / s[1] - log(s[1] / s[2]));
    grad[0] = 1 / ((ω - 1) * s[1]);
...
template <typename T>
void higher_correction_exp(T* H, T* z, T* η, T* ds, T* v) {
    T cholH[9] = {0};
...
template <typename T>
void update_dual_grad_H_exp(T* grad, T* H, T* z) {
    T l = log(-z[2] / z[0]);
    T r = -z[0] * l - z[0] + z[1];
